...
import { Calendar, Rocket, Clock, BarChart3, Globe, CalendarClock,  History,
} from "lucide-react";
export default function Dashboard() {
  ...
  const handleDeleteEvent = async (id) => {
    ...
    try { await axios.delete(`/events/${id}`);
  ...
  const statCards = [
    { label: "Total Events", value: events.length, color: "from-blue-500 to-cyan-400", icon: <Calendar size={26} />, },
    { label: "Upcoming", value: events.filter((e) => new Date(e.date) > now).length,  color: "from-green-500 to-emerald-400",
      icon: <Rocket size={26} />,},
    { label: "Past Events", value: events.filter((e) => new Date(e.date) <= now).length, color: "from-purple-500 to-fuchsia-400",
      icon: <Clock size={26} />,},
    {  label: "This Month", value: events.filter((e) => { const d = new Date(e.date);
        return (     d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear() ); }).length,
      color: "from-amber-400 to-orange-500", icon: <BarChart3 size={26} />,
    },
  ];

  const filters = [ { key: "all", label: "All", icon: <Globe size={18} /> },
        ...
        <header className="flex flex-col sm:flex-row sm:justify-between mb-10">
          <div>
            <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400">
              🌟 Events Dashboard  </h1>
            <p className="text-neutral-400">  Manage and explore your events with style ⚡ </p>  </div>
          <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {statCards.map(({ label, value, color, icon }) => (
            ...
            >
              ...
              <div className={`absolute inset-x-0 bottom-0 h-0.5 bg-gradient-to-r ${color} opacity-60 rounded-b-2xl`}
              />
            </div>
          ))}
        </div>
